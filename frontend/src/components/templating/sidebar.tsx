      ...
      <div className="px-6 py-8 space-y-10">
        <section className="space-y-4">
          <div className="flex flex-col gap-1">
            <h3 className="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Node Configuration</h3>
          ...
          <div className="flex gap-2 pb-2">
            <Button 
              size="sm" 
              className="flex-1 bg-primary text-primary-foreground hover:bg-primary/90 h-8 text-[11px] font-bold"
            ...
            <Button 
              size="sm" 
              variant="outline"
              className="flex-1 border-border bg-transparent hover:bg-accent h-8 text-[11px] font-bold"
          ...
          <div className="space-y-4 pt-2">
            <div className="flex w-full gap-4">
              <div className="grid gap-2 flex-1">
                <Label className="text-[10px] uppercase text-muted-foreground font-bold ml-1">Label</Label>
                <Input 
                  className="h-9 bg-muted/30 border-border text-foreground focus-visible:ring-1 focus-visible:ring-ring" 
                  value={template.label} 
                  onChange={(e) => updateRootField("label", e.target.value)} 
                />
              </div>
              <div className="grid gap-2 flex-1">
                <Label className="text-[10px] uppercase text-muted-foreground font-bold ml-1">Type ID</Label>
                <Input 
                  className="h-9 bg-muted/30 border-border text-foreground focus-visible:ring-1 focus-visible:ring-ring font-mono text-xs" 
                  value={template.type} 
            ...
            <div className="grid gap-2">
              <Label className="text-[10px] uppercase text-muted-foreground font-bold ml-1">Description</Label>
              <Textarea 
                className="min-h-[60px] bg-muted/30 border-border text-foreground resize-none focus-visible:ring-1 focus-visible:ring-ring" 
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Inputs</h3>
            <Button onClick={addParameter} variant="outline" className="h-7 text-[10px] bg-transparent border-border text-muted-foreground hover:bg-accent">
          ...
          <div className="space-y-3">
            {template.schema.parameters.map((param) => (
              <Card key={param.id} className="bg-muted/20 border-border/60 p-4 space-y-4 shadow-none">
                <div className="flex items-center justify-between gap-4">
                  <Input 
                    className="h-6 p-2 border-none bg-transparent text-sm font-medium text-foreground focus-visible:ring-0" 
                    value={param.label} 
                    onChange={(e) => updateParameter(param.id, { label: e.target.value })}
                  />
                  <Button onClick={() => removeParameter(param.id)} variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive">
                    <Trash2 className="w-3.5 h-3.5" />
                  </Button>
                </div>
                <div className="grid grid-cols-1 gap-3">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[11px] text-muted-foreground">Slug ID</span>
                    <Input 
                      className="h-6 w-[120px] bg-background border-border text-[10px] font-mono"
                      value={param.id}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Select value={param.type} onValueChange={(v: any) => updateParameter(param.id, { type: v })}>
                      <SelectTrigger className="h-7 w-[100px] text-[11px] bg-background border-border text-foreground">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-muted-foreground">Allow Connection</span>
                ...
                {param.type === "enum" && (
                  <div className="pt-3 border-t border-border space-y-3">
                    <div className="flex flex-wrap gap-1.5">
                      {param.values?.map((v) => (
                        <Badge key={v} variant="secondary" className="rounded px-2 py-0.5 text-[10px] flex items-center gap-1.5" onClick={() => updateParameter(param.id, { values: param.values?.filter(x => x !== v) })}>
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Button size="icon" variant="outline" className="h-8 w-8 shrink-0 border-border" onClick={() => handleAddEnum(param.id)}>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Outputs</h3>
            <Button onClick={addOutput} variant="outline" className="h-7 text-[10px] bg-transparent border-border text-muted-foreground hover:bg-accent">
              <Plus className="w-3 h-3 mr-1" /> Add
            </Button>
          </div>
          <div className="space-y-2">
            {template.schema.outputs.map((output) => (
              <Card key={output.id} className="bg-muted/20 border-border/60 p-3 space-y-3 shadow-none">
                <div className="flex items-center justify-between gap-2">
                  <Input 
                    className="h-6 p-2 border-none bg-transparent text-sm font-medium text-foreground focus-visible:ring-0" 
                    value={output.label} 
                  ...
                  />
                  <Button onClick={() => removeOutput(output.id)} variant="ghost" size="icon" className="h-5 w-5 text-muted-foreground hover:text-destructive">
                    <Trash2 className="w-3 h-3" />
                  </Button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <Label className="text-[9px] uppercase text-muted-foreground/60 font-bold ml-1">Slug ID</Label>
                    <Input 
                      className="h-7 bg-background border-border text-[10px] font-mono"
                      value={output.id}
                      onChange={(e) => updateOutput(output.id, { id: translateToSafeType(e.target.value) })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-[9px] uppercase text-muted-foreground/60 font-bold ml-1">Type</Label>
                    <Input 
                      className="h-7 bg-background border-border text-[10px]"
                      value={output.type}
